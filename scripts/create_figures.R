...
for(output_string in c("PRETERM", "STILLBIRTH")){
  for(add_str in c("", "SMOTE_")){
    ...
    colnames(beta_LOR)[2] <- "Mean"; rownames(beta_LOR) <- c(var_individual, var_neighborhood)
    beta_LOR <- beta_LOR[rev(index),]
    assign(ifelse(add_str == "", "orig_beta_LOR","SMOTE_beta_LOR"),beta_LOR)
  }
  res <- cbind(orig_beta_LOR, SMOTE_beta_LOR)
  assign(ifelse(output_string == "PRETERM", "res_pre","res_still"),res)
}

### Let's make the plot
### Run the following twice (one for stillbirth and one for preterm birth), everytime commenting out one of these two lines:

# res <- res_still; filename_str <- "cov_coeff_still_SMOTE.png"
res <- res_pre; filename_str <- "cov_coeff_pre_SMOTE.png"
...
png(filename = filename_str, width = 6, height = 7, units = "in", res = 300)
par(mar=c(3,10,4,1)+0.1,mgp=c(5,1,0))
plot(0, 0, xlim = xlim, ylim = ylim, xlab = "", ylab = "",
     axes = FALSE, type = "n", las = las)    
abline(v = 0, lty = 2, lwd = 1)
axis(1)
axis(2, at = (1:k)[-15], labels = labels[-15], las = las)
box()
legend("topright", c("CAR","CAR-SMOTE"), col = 1:2, lty = rep(1,2), pch = rep(pchs[1],2))
...
for(output_string in c("PRETERM", "STILLBIRTH")){
  for(add_str in c("", "a-reweight_")){
    ...
    colnames(beta_LOR)[2] <- "Mean"; rownames(beta_LOR) <- c(var_individual, var_neighborhood)
    beta_LOR <- beta_LOR[rev(index),]
    assign(ifelse(add_str == "", "orig_beta_LOR","reweight_beta_LOR"),beta_LOR)
  }
  res <- cbind(orig_beta_LOR, reweight_beta_LOR)
  assign(ifelse(output_string == "PRETERM", "res_pre","res_still"),res)
}
...
# res <- res_still; filename_str <- "cov_coeff_still_reweight.png"
res <- res_pre; filename_str <- "cov_coeff_pre_reweight.png"
...
axis(2, at = (1:k)[-15], labels = labels[-15], las = las)
box()
legend("topleft", c("CAR","CAR-Reweight"), col = 1:2, lty = rep(1,2), pch = rep(pchs[1],2))
title("Log odds-ratio")
